...
import { buildOrderSocketUrl } from "../api/orders";
import { useAuthStore } from "../store/authStore";

const RECONNECT_DELAY_MS = 3000;
const KEEPALIVE_INTERVAL_MS = 25000;
const TERMINAL_CLOSE_CODES = new Set([1000, 1001, 4400, 4401, 4403, 4404]);
...
const startKeepalive = (entry) => {
  ...
  entry.keepaliveTimer = window.setInterval(() => {
    ...
    try {
      ...
      // close handler will recover if needed
    }
...
const connectSocket = (key, entry) => {
  ...
  const socket = new WebSocket(buildOrderSocketUrl({ token: entry.token, orderId: entry.orderId }));
  entry.socket = socket;
  ...
  socket.onmessage = (event) => {
    try {
      ...
      entry.subscribers.forEach(({ onMessageRef }) => onMessageRef.current?.(payload));
    } catch (error) {
      console.error("Invalid order socket payload", error);
    }
  };
...
const subscribeToConnection = ({ token, orderId, onMessageRef, onStateChange }) => {
  const key = `order:${orderId}:${token}`;
...
export const useOrderRealtime = ({ orderId, enabled = true, onOrderEvent }) => {
