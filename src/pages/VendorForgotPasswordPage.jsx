...
export const VendorForgotPasswordPage = () => {
  ...
  return (
    <AuthLayout
      title={submitted ? 'Check Your Email' : 'Reset Your Merchant Password'}
      subtitle={
        submitted
          ...
          {mutation.error && (
            <div className="bg-error/10 border border-error/20 rounded-xl p-4">
              <p className="text-sm text-error font-medium">
                {mutation.error.response?.data?.detail ?? "Unable to generate token"}
              </p>
            </div>
          )}
        ...
        <>
          <div className="bg-primary-container/10 border border-primary/20 rounded-xl p-6 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-on-surface mb-1">Reset token sent to:</p>
